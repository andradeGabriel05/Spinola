...
import "./commentAllezVous.scss";
import axios from "axios";
import {
  getClickedWord,
  getValorKey,
  wordsForTranslate,
} from "./wordsForTranslate";

export default function CommentAllezVous() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (videoRef.current.currentTime > 264.37) {
        window.location.href = "/video-finish";
        localStorage.setItem("typeOfMedia", 1); // video
        localStorage.setItem("timeSpent", 264370);
        return;
      }
    ...
    <div className="container_music_video">
      <div className="wrapper_video_choice">
        <video id="videoMusic" autoPlay={true} ref={videoRef}>
          <source src="/CommentAllezVous.mp4" type="video/mp4" />
        </video>
